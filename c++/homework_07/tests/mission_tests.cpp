...
TEST(Factory, CreatesSolverNotNull) {
    IBallisticSolver* s = createSolver(SolverType::ANALYTICAL);
    EXPECT_NE(s, nullptr);
    delete s;
}

TEST(Factory, CreatesLoaderNotNull) {
    IConfigLoader* l = createLoader(LoaderType::FILE);
    EXPECT_NE(l, nullptr);
    delete l;
}
